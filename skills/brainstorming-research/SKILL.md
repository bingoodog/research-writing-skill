...
description: Use before writing any paper or project document - clarifies document type, scope, audience, and structure through natural dialogue
---

# 写作头脑风暴

通过自然的协作对话，帮助用户将论文或项目文档的想法转化为完整的写作计划。
...
| 文档类型 | 默认语言 | 正文列表 |
|----------|----------|----------|
| 本科/硕士/博士毕业论文 | 中文 | 不允许（除非用户要求）|
| 中文核心期刊 | 中文 | 不允许 |
| SCI/SSCI期刊 | 英文 | 不允许 |
| 会议论文 | 英文 | 不允许 |
| 课程论文 | 中文 | 不允许 |
| 软件需求规格说明书（SRS）| 中文 | 允许（需求编号列表）|
| 概要/详细设计文档 | 中文 | 允许（接口、步骤）|
| 项目可行性报告 | 中文 | 允许（风险矩阵、对比表）|
| 项目结题/总结报告 | 中文 | 允许（交付物清单）|
| 项目进展报告 | 中文 | 允许（任务列表）|
| 系统架构文档 | 中文 | 允许（模块列表）|
| 测试报告 | 中文 | 允许（缺陷列表）|

**不需要主动询问语言**，根据文档类型自动确定。只有当用户有特殊语言需求时才调整。

## 已有信息提示

在开始问答前，检查用户是否已有相关材料：

> "在开始之前，如果你已经有相关材料（比如题目、导师/甲方要求、现有文档草稿），可以现在发给我，我会根据这些信息来规划。如果没有也没关系，我们从头开始讨论。"
...
> - 测试报告
>
> 不同类型的文档在篇幅、结构和语言风格上会有差异，我需要先了解这个来帮你规划。"

**等待用户回答。** 收到回答后，确认你的理解：

> "好的，[文档类型]。这类文档通常使用 [引用/规范格式]，写作风格偏向 [风格特点]。"

然后进入下一个问题。

**注意**：如果是项目文档类，后续问答跳过"学科领域"和"文献综述相关"问题，改为询问项目背景、受众和约束。
...
### 项目文档专用问答（仅当文档类型为项目文档时执行）

如果用户选择的是**项目文档类型**（SRS、设计文档、可行性报告、结题报告、进展报告、架构文档、测试报告），跳过学科领域/研究方法问题，改为以下问答：

**问题A：项目背景与目标**

> "这个文档是为哪个项目写的？
>
> 简单描述一下项目是做什么的，以及这份文档要解决什么问题或说明什么内容？"
...
> "这份文档主要给谁看？
>
> 比如：开发团队、产品经理、项目甲方、上级领导，还是兼而有之？
>
> 我的建议是 [根据文档类型推荐受众]，你确认还是有调整？"
...
> "有没有需要遵循的格式要求或约束？
>
> 比如：公司文档模板、行业标准（如GB/T、IEEE）、字数要求、截止时间等。
>
> 没有的话直接告诉我，我用通用结构。"
...
- **语言**：[中文/英文]
- **格式标准**：[GB/T / IEEE / 公司模板 / 通用]
- **写作模块**：writing-project
```
...
> 你想从哪一节开始写？
>
> [论文]我的建议是先写绪论，把研究问题和目标明确下来，这样后面的章节写起来会更顺。
> [项目文档]我的建议是先写执行摘要或第一章，把文档目标和范围确定下来，再展开后面的内容。
>
> 你想先写哪部分？"

**等待用户回答**，然后根据文档类型调用对应技能：
- **论文** → 调用 `writing-chapters` 技能
- **项目文档** → 调用 `writing-project` 技能
